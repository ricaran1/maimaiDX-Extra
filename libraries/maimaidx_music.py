    ...
    def by_level_list(self) -> Dict[str, Dict[str, List[RaMusic]]]:
        ...
        for music in self:
            ...
            for index, ds in enumerate(music.ds):
                ...
                try:
                    level_key = music.level[index]  # 例如 "9+"
                    ds_key = str(ds)                # 例如 "9.6"
                except IndexError:
                    continue  # 跳过 level/ds 不匹配的数据
                ...
                if level_key not in _level:
                    _level[level_key] = {}
                if ds_key not in _level[level_key]:
                    _level[level_key][ds_key] = []

                _level[level_key][ds_key].append(ra)
