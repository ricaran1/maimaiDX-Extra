...
from .maimaidx_model import ChartInfo, PlayInfoDefault, PlayInfoDev, UserInfo,PC_UserInfo,PC_ChartInfo
...
class ScoreBaseImage:
    ...
    def whiledraw(
        ...
        for num, info in enumerate(data):
            ...
            self._tb.draw(x + 93, y + 38, 28, f'{info.achievements:.4f}%', self.t_color[info.level_index], anchor='lm')
    ...
    def while_pc_draw(
        self, 
        data: Union[List[PC_ChartInfo], List[PlayInfoDefault], List[PlayInfoDev]], 
        dx: bool, 
        height: int = 0
    ) -> None:
        """
        ...
        # y为第一排纵向坐标，dy为各行间距
        dy = 114
        if data and type(data[0]) == PC_ChartInfo:
            y = 1085 if dx else 235
        else:
            y = height
        for num, info in enumerate(data):
            ...
            self._sy.draw(x + 93, y + 14, 14, title, self.t_color[info.level_index], anchor='lm')
            self._tb.draw(x + 93, y + 38, 28, f'{info.achievements:.4f}%', self.t_color[info.level_index], anchor='lm')
            self._tb.draw(x + 219, y + 65, 15, f'counts:{info.playCount}', self.t_color[info.level_index], anchor='mm')
            self._tb.draw(x + 93, y + 65, 15, f'{info.ds} -> {info.ra}', self.t_color[info.level_index], anchor='lm')
...
class DrawBest(ScoreBaseImage):
    ...
    async def draw(self) -> Image.Image:
        ...
        self._sy.draw(
            700, 1570, 27, 
            f'Designed by Yuri-YuzuChaN & BlueDeer233 | Generated by 舞萌痴bot | Powerd by Ricaran', 
...
class Draw_PCBest(ScoreBaseImage):
    ...
    async def draw(self) -> Image.Image:
        ...
        self.while_pc_draw(self.sdBest, False)
        self.while_pc_draw(self.dxBest, True)
...
async def generate(qqid: Optional[int] = None, username: Optional[str] = None) -> Union[MessageSegment, str]:
    ...
    try:
        ...
        msg = f'未知错误：{type(e)}\n请联系Bot管理员'
        if msg=="未知错误：<class 'httpx.ReadTimeout'>\n请联系Bot管理员":
            msg=f'获取成绩超时，可能是水鱼查分器爆炸\n请稍后再试！'
    return msg
...
async def generate_other50(userdata,qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        ...
        if msg=="未知错误：<class 'httpx.ReadTimeout'>\n请联系Bot管理员":
            msg=f'获取成绩超时，可能是水鱼查分器爆炸\n请稍后再试！'
    return msg
async def generate_pc50(userdata,qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None

        mai_info = PC_UserInfo(**userdata)
        draw_best = Draw_PCBest(mai_info, qqid)
